#!/usr/bin/env python3
"""Maqueta "chulo" (premium) de Cuentos de Tarel para tapa dura KDP, 6 x 9".

Adaptación directa de edicion_ensayo/scripts/build_interior_premium.py (la
misma maqueta con alma que edicion_poesia/scripts/build_interior.py:
...
El cuerpo de cada cuento (párrafos, diálogo, citas, ilustraciones en línea)
reutiliza tal cual el parser markdown del motor genérico
(generate_book_pdf.markdown_to_flowables).

    python3 edicion_cuentos/scripts/build_interior_premium.py \
        ../toc_cuentos.json -o ../Cuentos_de_Tarel_6x9.pdf
"""
...
# ---------------------------------------------------------------- geometria
PW, PH = 6 * inch, 9 * inch
# KDP exige medianil segun el numero de paginas del libro (0.5" para
# 151-300, la banda donde cae este volumen) pero su comprobador automatico
# rechaza texto que llega exactamente a ese minimo (lo mide sobre el PDF
# rasterizado y no da margen de tolerancia) -- de ahi el colchon extra
# sobre el minimo publicado. Reajustar si el recuento final de paginas
# cambia de banda.
M_GUTTER = 0.55 * inch
M_OUTER = 0.625 * inch
M_TOP = 0.68 * inch
...
def dedication_page(story: list) -> None:
    ...
    story.append(Spacer(1, TEXT_H * 0.42))
    story.append(Paragraph(
        f'<para alignment="center"><i>{tr("dedication")}</i></para>',
...
def toc_page(story: list) -> None:
    ...
    story.append(ForceParity(1, TEXT_H))
    story.append(SmallCapsFlowable(tr("toc_title"), R, 15, INK, spacing=4.0, space_after=16))
    toc = TableOfContents()
...
def colophon_page(story: list) -> None:
    ...
    story.append(Paragraph(
        f'<para alignment="center"><i>{tr("colophon")}</i></para>',
...
def chapter_label(chapter_number: Optional[str]) -> Optional[str]:
    ...
    # la coda) no llevan chapterNumber en absoluto -- el número de por sí no
    # aporta nada a una antología que ya se lee en su orden propio.
    return None
...
def build_pdf(toc_path: Path, output_path: Path, ca_bundle: Optional[str] = None) -> None:
    ...
    with toc_path.open("r", encoding="utf-8") as fh:
        toc = json.load(fh)
    global LANG, BOOK_TITLE
    LANG = toc.get("language", "es")
    BOOK_TITLE = toc.get("title", BOOK_TITLE).upper()
