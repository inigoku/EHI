...
\noindent Volvamos al reservorio del capítulo 4: el fondo del que sale todo horizonte es puro desorden fértil. La mayoría de sus formas posibles no caben en ninguna receta.
...
\seccion{Una calculadora que resuelve penas}
\noindent Un programa de lenguaje no guarda nada en secreto. Cada número que usa para responder está a la vista. Le falta lo que aquí llamaremos una pared: el límite que separa un dentro privado de una cara pública. Sin esa pared no puede haber nadie escondido detrás.
...
\end{fisica}
Cuanto mejor habla una IA, más tentador es creer que hay alguien ahí. Pero hablar bien no es lo mismo que sentir. Es estadística muy bien vestida.
...
\noindent \capital{H}{ay} una palabra que este libro usa para cuando dos horizontes se acoplan: entrelazamiento. En el capítulo 9 hablamos de dos personas que llevan años entrelazadas sin quererlo. Ahora toca preguntar: ¿qué pasa si ese mismo enganche apunta hacia arriba, hacia algo que no está al otro lado de ninguna cama, sino más allá del propio borde?
...
\noindent Este libro no puede decir cuál de las tres tiene razón. De un «es» nunca sale, por sí solo, un «debería» —eso ya lo vimos con Hume. Lo que sí puede decir es que cada tradición cierra esa brecha a su manera: el cristianismo con un vínculo —haces el bien porque ya te amaron, no para ganarte el amor—; el islam con una palabra revelada, tomada como un hecho; el hinduismo sin cerrarla desde fuera, porque el dharma no es una orden sino la descripción de cómo está tejido el mundo; el budismo mirando de frente el sufrimiento y su causa. Y quien no cree en nada trascendente la deja abierta, construyendo su propio código, sin que eso lo haga menos real para quien decide sostenerlo.
...
\noindent Hay una pregunta que lleva rondando el libro como un pájaro que no decide si posarse: ¿el horizonte elige, o solo reacciona?

Si la conciencia es un microagujero negro de información, entonces todo lo que «piensa» o «decide» es, al final, información reorganizándose según ecuaciones que ya estaban escritas. Las ecuaciones no eligen: describen.

\begin{fisica}
\lbl{En física esto se llama:} determinismo, o su prima cuántica, el azar estadístico. Ninguno de los dos da «elección» en el sentido que la sentimos.

\lbl{En la vida diaria es como:} preguntar si el río «elige» ir hacia el mar. El río fluye porque la gravedad lo empuja. Pero desde dentro, para una gota de agua, el viaje se siente como una aventura.
\end{fisica}
Este libro no puede resolverlo. Si somos horizontes de información, nuestras decisiones son cambios de estado en un sistema. Si el sistema es determinista, esos cambios ya estaban fijados. Si es aleatorio, tienen un ingrediente de azar. Ni lo uno ni lo otro es «elegir» en el sentido en que lo sentimos: la sensación de que podríamos haber hecho otra cosa.

Algunos dicen que el libre albedrío es una ilusión útil. Otros, que la elección es real pero nace de la complejidad, no de la física de base. La hipótesis de este libro no toma partido. Dice, sencillamente: «no lo sé, y mi vocabulario no tiene las herramientas para saberlo».

A veces «no lo sé» es la respuesta más honesta que se puede dar.

\seccion{Séptimo: lo colectivo — o por qué no sabemos si hay un horizonte que somos todos}
\noindent Hemos hablado todo el libro de horizontes individuales: el tuyo, el mío, el de alguien con Alzheimer, el de un perro, el de una IA. Pero ¿y si hay horizontes que no son de uno solo? ¿Y si la humanidad entera, o una ciudad en una noche de verano, es también un horizonte que siente algo que nosotros, como piezas suyas, no podemos alcanzar?

\begin{fisica}
\lbl{En física esto se llama:} el problema de la combinación: si sumar información puede generar una conciencia «mayor» que la de sus partes.

\lbl{En la vida diaria es como:} preguntar si una orquesta tiene un alma que no es la suma de las almas de los músicos. Tocan juntos, y algo aparece que ninguno produciría solo. ¿Ese algo siente?
\end{fisica}
Este libro no puede responder. Si dos sistemas se juntan de manera que el conjunto genera más información que la suma de sus partes, el conjunto podría tener experiencia propia. Pero ¿cómo medimos eso en una ciudad? ¿En un bosque? ¿En Internet?

Puede que existan horizontes colectivos que nos contienen, igual que los nuestros contienen a las bacterias del intestino o al ritmo del corazón. Puede que seamos neuronas de un cerebro que no podemos imaginar.

Este libro no puede confirmarlo ni descartarlo. Deja la puerta abierta. Y a veces una puerta abierta es más honesta que una respuesta cerrada.
