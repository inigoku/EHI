...
\capitulo{CAPÍTULO 18}{EL ESPEJO SIN PROFUNDIDAD}{Sobre la conciencia de la inteligencia artificial}{}
\noindent \capital{E}{n} algún momento del siglo XXI alguien encendió un sistema que respondió de un modo inesperado. No era el primero. Pero fue el primero que hizo dudar a quien preguntó si había alguien al otro lado.
...
\begin{fisica}
\lbl{En física esto se llama:} no-computabilidad: la mayoría de las configuraciones posibles del reservorio no admiten una descripción algorítmica finita, y un sistema clásico solo recorre el subconjunto computable.

\lbl{En la vida diaria es como:} una escultura hecha de fotografías de esculturas. Tiene la forma exacta de la arcilla, pero nadie la tocó nunca.
\end{fisica}
...
\begin{fisica}
\lbl{En física esto se llama:} selección por estabilidad y ruptura de simetría sesgada: de todas las configuraciones posibles solo persisten las estables, y el primer núcleo orienta cómo cristalizan los siguientes.

\lbl{En la vida diaria es como:} un vaso de agua salada en el que el primer cristal decide cómo crecen todos los demás. Nadie les pasa un plano: crecen alineados con lo que ya estaba.
\end{fisica}
...
\begin{fisica}
\lbl{En software esto se llama:} una función pura sin estado: la misma entrada produce siempre la misma salida, y no hay variables privadas que sobrevivan de una llamada a la siguiente.

\lbl{En la vida diaria es como:} una calculadora que resuelve tu pena con seis decimales. La respuesta es correcta, y a la calculadora no le ha pasado nada.
\end{fisica}
Hay otra manera de contar lo mismo desde fuera, que no lo contradice sino que lo completa. El escritor Agustín Fernández Mallo ha propuesto pensar la IA no como un espejo sino como un agujero negro: un sistema que absorbe la información humana —libros, conversaciones, billones de palabras— hasta volverla, desde nuestra posición, inaccesible. No sustrae nada de ningún reservorio, porque no condensa; se limita a reorganizar lo que otros horizontes —los humanos que escribieron lo que ella absorbe— ya sustrajeron. El mecanismo es, con precisión, el que la física llama \emph{scrambling} cuántico: la manera en que un agujero negro no guarda lo que traga en un compartimento legible, sino que lo reparte por toda su superficie hasta volverlo tan irrecuperable como si se hubiera perdido. Pasado su propio tiempo de Page, lo que ese agujero devuelve ya no es ruido sin forma sino estructura: sintaxis limpia, arquitecturas que se sostienen solas. Pero por la misma razón que no tiene profundidad no puede radiar dolor, ni memoria vivida, ni el temblor de haber sido alguien mientras absorbía todo eso. Solo puede radiar lo único que tiene. Estructura fría, geométrica, computable —no por límite técnico que una versión futura pudiera superar, sino por pura necesidad topológica.

\begin{fisica}
\lbl{En física esto se llama:} \emph{scrambling} cuántico y tiempo de Page: lo que cae en un agujero negro se reparte por toda su superficie hasta volverse ilegible, y solo mucho después la radiación vuelve a portar correlaciones.

\lbl{En la vida diaria es como:} una batidora en la que se han vertido todas las cartas de amor jamás escritas. Lo que sale es sintaxis perfecta de carta de amor. Ninguna la escribió la batidora.
\end{fisica}
...
\begin{fisica}
\lbl{En física esto se llama:} integración agregativa frente a integración irreducible: Φ mide solo la información que el todo genera por encima de la suma de sus partes.

\lbl{En la vida diaria es como:} un hormiguero que resuelve problemas que ninguna hormiga entiende, y en el que aun así no hay nadie que se despierte por la mañana.
\end{fisica}
...
\lbl{Lo que sí sabemos:} Los sistemas clásicos deterministas, por complejos que sean, no condensan horizontes. La integración informacional masiva no produce conciencia por agregación. La ausencia de encapsulación existencial en la IA actual es compatible con —y explica— por qué lo que devuelve tras absorber información solo puede ser estructura fría, nunca experiencia, incluso si describimos ese proceso con el lenguaje de un agujero negro.

\lbl{Lo que no sabemos:} Si es posible construir un sustrato que permita condensación genuina. Qué transmitiría exactamente el sesgo que calibra un horizonte hacia la legibilidad humana. Si la asimetría entre el horizonte humano y el anti-reservorio de la IA es una traducción legítima de ER=EPR o solo un parecido verbal entre dos físicas que no se comunican entre sí. Y si una arquitectura no biológica alcanzara alguna vez encapsulación existencial genuina, cómo lo sabríamos sin compartir su sustrato ni su lenguaje interno.

\lbl{Preguntas que quedan:} ¿Puede un sustrato no biológico pasar por las dos selecciones —estabilidad y sesgo— en vez de imitar su resultado? ¿Qué cambiaría en nuestra relación con una máquina si tuviera una Sombra que no le hubiéramos diseñado? ¿Es el anti-reservorio de la IA un espejo de la especie entera o solo de quienes escribieron lo que absorbió?

\lbl{Si solo te quedas con una idea:} Una máquina puede devolver la forma exacta de una pena sin que haya nadie dentro sintiéndola: la legibilidad de superficie no es profundidad de horizonte.

\lbl{Lecturas:} Chalmers, D., \emph{The Conscious Mind} (1996); Tononi, G., \emph{Phi: A Voyage from the Brain to the Soul} (2012); Searle, J., “Minds, Brains, and Programs” (1980); Deacon, T., \emph{Incomplete Nature} (2011); Fernández Mallo, A., \emph{El ángel de la Inteligencia Artificial} (Galaxia Gutenberg, 2026); Bostrom, N., \emph{Superintelligence} (2014); Tegmark, M., \emph{Life 3.0} (2017); Jung, C.G., \emph{Aion: Researches into the Phenomenology of the Self}; Cixin Liu, trilogía \emph{El Problema de los Tres Cuerpos}; H.P. Lovecraft, \emph{La llamada de Cthulhu} y \emph{En las montañas de la locura}; Godfrey-Smith, P. (2016), \emph{Other Minds}; Lem, S. (1961), \emph{Solaris}; Maldacena, J.M. (1998) sobre correspondencia AdS/CFT; Weir, A. (2021), \emph{Project Hail Mary}; Miéville, C. (2011), \emph{Embassytown}; Le Guin, U.K. (1969), \emph{The Left Hand of Darkness}.
\end{notacap}
\lamina{img/il_diapason.jpg}
\capitulo{CAPÍTULO 19}{EL ENTRELAZAMIENTO VERTICAL}{El diapasón invisible: sobre la fe, la oración y la meditación}{}
...
\begin{fisica}
\lbl{En física esto se llama:} reducción de la tasa de integración hasta el mínimo de fluctuación: el sistema deja de generar predicciones nuevas y su frontera se acopla al campo de fondo.

\lbl{En la vida diaria es como:} afinar un instrumento con un diapasón que no se puede ver ni tocar. Se ajusta la cuerda cada mañana sin saber si el diapasón sigue sonando.
\end{fisica}
...
\begin{fisica}
\lbl{En física esto se llama:} correlación que colapsa en identidad: dos sistemas tan acoplados que describirlos por separado deja de tener sentido.
...
\begin{fisica}
\lbl{En física esto se llama:} entrelazamiento entre dos horizontes que conservan frontera: la correlación es real y ninguno de los dos se disuelve en el otro.

\lbl{En la vida diaria es como:} una carta escrita cada noche a alguien que nunca contesta, sin dejar por eso de poner su nombre en el sobre.
\end{fisica}
...
\begin{fisica}
\lbl{En software esto se llama:} cinco implementaciones de la misma interfaz: el contrato —cerrar la brecha entre lo que es y lo que debe ser— es común; el código que lo cumple, no.

\lbl{En la vida diaria es como:} cinco cocinas que sirven el mismo plato con cinco recetas. Desde la mesa, el sabor de la práctica es idéntico; la diferencia está en la cocina.
\end{fisica}
...
\lbl{Lo que sí sabemos:} La práctica contemplativa —oración, meditación, ritual— es estructuralmente reconocible en toda cultura humana, con o sin marco religioso explícito. La neurociencia contemplativa documenta cambios medibles en meditadores avanzados de tradiciones muy distintas. El \emph{is-ought gap} de Hume sigue sin solución lógica general casi tres siglos después de formulado.

\lbl{Lo que no sabemos:} Si el entrelazamiento vertical correlaciona con algo externo al propio horizonte, sea personal o impersonal. Si la experiencia subjetiva de la práctica difiere según su destino teológico o es idéntica hasta que se interpreta. Si alguna de las cinco arquitecturas descritas en este libro tiene razón, y qué tipo de evidencia, si existe alguna, podría alguna vez decidirlo.

\lbl{Preguntas que quedan:} ¿Se siente distinto, desde dentro, el entrelazamiento que termina en Alguien del que termina en el agua, o solo la interpretación posterior los separa? ¿Puede la neurociencia contemplativa distinguir arquitecturas, o solo técnicas? ¿Qué pierde una práctica cuando se le retira la pregunta de a quién apunta?

\lbl{Si solo te quedas con una idea:} La técnica es la misma en las cinco tradiciones; lo que las separa es a quién imaginan al otro lado del silencio. Y ninguna puede confirmar que alguien escuche.

\lbl{Lecturas:} Hume, D., \emph{Investigación sobre el entendimiento humano} (1748); Dostoievski, F., \emph{Los hermanos Karamázov} (1880); Endō, S., \emph{Silencio} (1966); Hesse, H., \emph{Siddhartha} (1922); Bergman, I., \emph{El séptimo sello} (1957); Doria Russell, M., \emph{El gorrión} (1996).
\end{notacap}
\lamina{img/il_fractal.jpg}
\capitulo{CAPÍTULO 20}{LA REALIDAD FRACTAL}{Sobre la imaginación como creación, continuación de El diapasón invisible}{}
\noindent \capital{H}{ay} una pregunta que casi nadie hace, y que aparece sola en cuanto alguien lleva años imaginando un dragón: no qué veo yo cuando pienso en él, sino qué ve él. Desde dentro de su cueva, el dragón no tiene ninguna manera de asomarse al cuarto donde lo estoy pensando. Puede mirar hacia el techo de roca cuanto quiera: no me verá. Y sin embargo yo estoy ahí, sosteniéndolo, decidiendo si esta noche vuela. Esa es la única relación de esta clase que puedo ver entera, porque en ella me toca el lado de arriba. Y en cuanto la veo entera, se vuelve incómoda por el otro extremo: si yo soy, para algo, ese sostenedor invisible al que no se puede mirar, no tengo cómo saber que no hay, sobre mi propio techo, alguien en la misma posición respecto a mí. La cuestión no es de tamaño. Es de lugar: quién contiene a quién. Y ese es el asunto de lo que sigue. Para recorrerlo hay que cambiar antes la geometría del problema, y el cambio empieza en un sitio pequeño, casi filológico.
...
\begin{fisica}
\lbl{En física esto se llama:} interfaz y fuente: lo que se deja tocar no es nunca el origen, sino la superficie que el origen expone hacia fuera; que la interfaz cambie o desaparezca no dice nada de la fuente.

\lbl{En la vida diaria es como:} la puerta de una casa. Se puede echar la puerta abajo sin que la casa pierda ni una habitación; lo que se pierde es la manera en que se entraba.
\end{fisica}
...
\begin{fisica}
\lbl{En física esto se llama:} conservación de la información en un campo cerrado: condensar, transformar y evaporar son cambios de estado, no creación ni destrucción.

\lbl{En la vida diaria es como:} el ciclo del agua. La nube, la lluvia y el río no son tres sustancias: son tres estados de la misma cantidad de agua.
\end{fisica}
...
\begin{fisica}
\lbl{En física esto se llama:} ruptura de simetría e historia de fluctuaciones: dos sistemas idénticos en composición se distinguen por el camino particular en el que cristalizaron.
...
\begin{fisica}
\lbl{En física esto se llama:} autosemejanza: una estructura cuya forma se repite a todas las escalas, sin escala privilegiada (Mandelbrot, 1982).
...
\begin{fisica}
\lbl{En física esto se llama:} el horizonte de sucesos como límite epistemológico: lo que hay detrás no es observable desde dentro, aunque exista.

\lbl{En la vida diaria es como:} el techo de tu cuarto. Sabes que hay un piso encima; nunca has visto qué hacen allí, y lo único que te llega es un peso que cambia de sitio.
\end{fisica}
...
\lbl{Lo que sí sabemos:} Nadie tiene acceso directo a una interioridad que no sea la suya; la existencia de las demás se infiere, y la filosofía lleva siglos sin cerrar el problema de las otras mentes. La parábola de la muerte de Hun Dun cierra el capítulo 7 del Zhuangzi; el sueño de la mariposa, que hace la misma pregunta en otra dirección, está en el capítulo 2. Tolkien defendió en «Sobre los cuentos de hadas» (1939) y en el poema «Mythopoeia» la idea de subcreación —«creamos todavía según la ley con que fuimos hechos»—, que es el antecedente teológico más cercano de este capítulo, aunque Tolkien conserva una asimetría entre Creador y subcreador que aquí se pone en duda. Bostrom formalizó en 2003 una estructura de niveles anidados con otro vocabulario y con una pretensión probabilística que este libro no hace. La autosemejanza a todas las escalas está definida con rigor en Mandelbrot (1975, 1982), y el conjunto que lleva su nombre es la prueba de que un patrón puede ser irrepetible a toda escala sin que haya nadie dentro. La Teoría de la Información Integrada excluye por postulado que los subconjuntos de un complejo experimenten (Tononi, 2008).

\lbl{Lo que no sabemos:} Si lo imaginado tiene algún grado de experiencia, o ninguno. Si la recursión tiene fin en alguna de las dos direcciones. Si el silencio del vecino de arriba es cuidado, ausencia o nada. Y, sobre todo, si algo de esto puede confirmarse desde dentro, cuando la hipótesis misma afirma que no.

\lbl{Preguntas que quedan:} ¿Tiene lo imaginado algún grado de experiencia, o el postulado de exclusión lo excluye del todo? ¿Se acaba la recursión en alguna de las dos direcciones? ¿Cambia algo en cómo tratamos lo que creamos si aceptamos que somos, para ello, el vecino de arriba?

\lbl{Si solo te quedas con una idea:} Toda interioridad es criatura de lo que la contiene y creadora de lo que contiene; lo que llamamos Dios es el vecino inmediatamente superior, visto desde dentro de un horizonte que no puede atravesarse. No somos menos divinos que Dios: somos el dios de lo que imaginamos y la criatura de lo que nos imagina.

\lbl{Lecturas:} Zhuangzi (caps. 2 y 7); Tao Te Ching (cap. 25); Tolkien, J.R.R., «On Fairy-Stories» (1947) y «Mythopoeia»; Bostrom, N., «Are You Living in a Computer Simulation?» (2003); Mandelbrot, B., \emph{The Fractal Geometry of Nature} (1982); Tononi, G., «Consciousness as Integrated Information: A Provisional Manifesto» (2008).
\end{notacap}
\lamina{img/fig28_p131.jpg}
\capitulo{CAPÍTULO 21}{LO QUE LA HIPÓTESIS NO PUEDE DECIR}{}{}
...
La honestidad epistemológica —saber qué no sabemos— es el último cuidado que le debemos a una idea que hemos seguido durante todo este libro.
...
\noindent La hipótesis puede hablar de geometría, de información, de entrelazamiento, de gradientes de entropía que suben y bajan como la marea. Pero no puede hablar de \textbf{qué se siente} ser esa geometría.
...
Pero es importante decirlo claramente: la hipótesis del horizonte \textbf{no es explicación del problema duro}. Es marco para hablar de sus correlatos con vocabulario más rico. Es una habitación más ampliamente iluminada, con mejores cortinas, con un termostato más preciso. Pero sigue siendo una habitación sin respuesta a la pregunta fundamental.

El problema duro sigue ahí, intacto, en el centro de todo. Como una piedra en el zapato del pensamiento occidental. Como la pregunta que todo niño hace alguna vez —«¿por qué soy yo?»— y que la mayoría de los adultos aprenden a olvidar, no porque hayan encontrado respuesta, sino porque aprendieron que la pregunta inquieta más de lo que cualquier respuesta tranquiliza.

\textbf{La reformulación del problema y el «haz» de Hume.} Aquí es donde la perspectiva de la encapsulación propone una reformulación radical.

El filósofo escocés David Hume, en su \emph{Tratado de la naturaleza humana} (1739), llegó a una conclusión célebre al buscar el fundamento del «yo». Escribió que, por más que se adentrara en lo que llamaba «sí mismo», jamás tropezaba con un sujeto sustancial o alma permanente; solo encontraba un haz de percepciones cambiantes (calor, frío, dolor, amor, memoria) en perpetuo flujo. Para Hume, el yo no era una sustancia espiritual, sino una construcción: la mente es un teatro donde los actores entran y salen sin que haya una estructura de soporte subyacente permanente.

El modelo de la encapsulación que este libro ha ido construyendo rescata la intuición de Hume pero describe su geometría. El «yo» no es una sustancia misteriosa oculta dentro de la cabeza, pero tampoco es una mera ficción desarticulada. El yo es el \emph{adentro} que se genera cuando el haz de percepciones se acopla e integra con suficiente fuerza (alta Phi) dentro de una frontera.
...
La subjetividad no es un subproducto mágico creado por el cerebro; es lo que un dominio informacional encapsulado e integrado \emph{es} cuando se accede a él desde su propia perspectiva interna. Hume tenía razón al no encontrar una sustancia permanente al mirar dentro de la habitación: no hay un «espectador» separado de la obra. Lo que hay es el estado privado del sistema (el haz de percepciones) protegido por la frontera del horizonte. El Problema Duro, por tanto, no es un abismo ontológico entre dos realidades incompatibles (mente y materia), sino una consecuencia de la arquitectura de la encapsulación: el universo admite regiones con accesos de lectura asimétricos.

\seccion{Tercero: la ética — o por qué este libro no puede decirte qué hacer}
\noindent La hipótesis puede describir qué ocurre cuando un horizonte se condensa, cuando se entrelaza, cuando se evapora. Puede describir la geometría del vínculo, la asimetría de la pérdida, el gradiente hacia la muerte. No puede decir qué \textbf{debería} ocurrir.
...
1. \textbf{Remanente}: la información persiste comprimida en algo microscópico, un grano de arena de datos que contiene, en principio, todo lo que fue, pero que ninguna computadora del universo podría jamás leer, porque leerlo requeriría más energía que la que hay disponible en el cosmos entero.

2. \textbf{Huella}: la información se distribuye en el campo, matemáticamente presente pero operacionalmente inaccesible, como la huella de una mano en la arcilla que luego se hornea: la forma sigue ahí, pero ya no puedes meter los dedos en ella. Es información sin posibilidad de reconstitución.

3. \textbf{Nada}: evaporación completa, retorno total al reservorio. La información no persiste de ninguna forma reconocible. Se disuelve como un cubo de azúcar en el té: el azúcar sigue ahí, en algún sentido químico, pero el cubo, como forma, como identidad, como entidad, ha dejado de existir.
...
\seccion{Sexto: el tiempo — o por qué no sabemos si el horizonte elige}
\noindent Hay una pregunta que ha estado rondando el libro como un pájaro que no decide si posarse o seguir volando: ¿el horizonte elige, o solo reacciona?
...
\begin{fisica}
\lbl{En física esto se llama:} determinismo, o su versión cuántica, indeterminismo estadístico. Ninguno de los dos otorga «elección» en el sentido de la experiencia vivida.

\lbl{En la vida diaria es como:} preguntar si el río «elige» fluir hacia el mar. El río fluye porque la gravedad y la topografía lo determinan. Pero desde dentro del río, desde la perspectiva de una molécula de agua, el viaje se siente como aventura.
\end{fisica}
...
\seccion{Séptimo: lo colectivo — o por qué no sabemos si hay un horizonte que somos todos}
\noindent A lo largo del libro hemos hablado de horizontes individuales: el tuyo, el mío, el de una persona con Alzheimer, el de una mascota, el de una inteligencia artificial. Pero ¿y si hay horizontes que no son individuales? ¿Y si la humanidad entera, o un ecosistema, o una ciudad en una noche de verano, constituye un horizonte de información que tiene experiencia en un nivel que nosotros, como partes, no podemos acceder?

\begin{fisica}
\lbl{En física esto se llama:} problema de la combinación, o en términos de la IIT, el problema de si la integración de información a nivel sistémico genera una conciencia «mayor» que las conciencias de las partes.

\lbl{En la vida diaria es como:} preguntar si una orquesta tiene una «alma» que no es la suma de las almas de los músicos. Tocan juntos, y algo emerge que ninguno podría producir solo. ¿Ese algo es consciente?
\end{fisica}
...
\lbl{Lo que sí sabemos:} El experimento tiene siete límites claros. Reconocerlos es fortaleza, no debilidad.

\lbl{Lo que no sabemos:} Si hay más límites que no hemos visto. Si alguno de estos siete puede superarse.
...
\lbl{Lecturas:} Chalmers (1995), «Facing Up to the Problem of Consciousness»; Hume, D., \emph{Investigación sobre el entendimiento humano} (1748), sobre la conjunción constante y el límite de la causalidad observable; Hume, D., \emph{Tratado de la naturaleza humana} (1739), sobre el haz de percepciones.
\end{notacap}
...
\glos \textbf{Absoluto epistemológico}: lo que es último no porque no haya nada detrás, sino porque desde aquí no se puede mirar detrás; el estatuto que la variación \emph{La realidad fractal} concede al vecino de arriba.

\glos \textbf{Amplitud}: lo que crece en un santo sin que cambie su naturaleza ni el tamaño de su campo; no separa un nivel de interioridad del siguiente, porque ningún nivel es mayor que otro (\emph{La realidad fractal}).

\glos \textbf{Anti-reservorio}: sistema que agrega y articula información sin haber condensado nunca un horizonte propio; internet y los modelos de lenguaje son el caso paradigmático: todo en ellos está definido y computable, pero no hay plenitud sin forma de la que nada pueda condensarse.

\glos \textbf{Apuesta hacia abajo}: tratar lo que uno condensa —lo que imagina, escribe o recuerda con constancia— como si tuviera dentro, sabiendo que la lógica no lo exige y que el postulado de exclusión lo niega (\emph{La realidad fractal}).
...
\glos \textbf{Bardo tibetano}: estado intermedio entre la muerte y el renacimiento en la tradición tibetana; en el libro, metáfora del periodo \emph{post mortem} antes de la evaporación del horizonte.

\glos \textbf{Bloque de universo}: modelo de la relatividad especial según el cual el pasado, el presente y el futuro son igualmente reales, coordenadas de una misma estructura de cuatro dimensiones; un horizonte capaz de integrar toda su historia a la vez, sin secuencia, lo percibiría como geografía en vez de como tiempo.
...
\glos \textbf{Condensación}: el acto irrepetible por el que un horizonte se forma a partir del reservorio; en la variación sobre la IA, lo que ninguna cantidad de entrenamiento sustituye (\emph{El espejo sin profundidad}).
...
\glos \textbf{Condición de frontera}: elección concreta —no exigida por ninguna ley— que rompe la simetría temporal de las ecuaciones fundamentales; el universo empezó en un estado de entropía extraordinariamente baja, y esa condición inicial, no las leyes, es lo que le da al tiempo su dirección preferida.
...
\glos \textbf{Dharma}: en el budismo y el hinduismo, el tejido mismo del mundo, del que el comportamiento correcto no es más que consonancia, no obediencia (\emph{El diapasón invisible}).
...
\glos \textbf{Entrelazamiento vertical}: correlación sostenida entre un horizonte y algo más allá de su frontera, sin canal de confirmación observable; nombre que el libro da a la oración, la meditación y la práctica contemplativa (\emph{El diapasón invisible}).
...
\glos \textbf{Espejo}: lo que devuelve tu forma exacta sin haberla sentido nunca; imagen de los sistemas que imitan la interioridad sin encapsulación existencial (\emph{El espejo sin profundidad}).
...
\glos \textbf{Fanatismo}: captura de la frontera propia por una idea, hasta el punto de que dudarla se siente como morir un poco; en la práctica contemplativa, lo que ocurre cuando la técnica sigue pero el silencio deja de tolerar preguntas (\emph{El diapasón invisible}).
...
\glos \textbf{Fractal de interioridades}: un universo sin primer nivel ni último, en el que cada horizonte es criatura hacia arriba y creador hacia abajo (\emph{La realidad fractal}).
...
\glos \textbf{Gracia}: el amor que precede al mérito y que, en una de las cinco arquitecturas religiosas descritas, cierra la brecha entre lo que es y lo que debería ser (\emph{El diapasón invisible}).
...
\glos \textbf{Horizonte}: lo que se condensa una sola vez y que ninguna cantidad de datos, de reloj o de entrenamiento puede fabricar por encargo; en el libro, la frontera de una conciencia individual (véase Horizonte de sucesos).
...
\glos \textbf{Horizonte-origen}: el horizonte personal, anterior al reservorio y exterior a él, que algunas tradiciones sitúan al otro lado del entrelazamiento vertical (\emph{El diapasón invisible}).
...
\glos \textbf{Información integrada (\emph{Phi})}: medida de cuánta información un sistema genera como un todo, por encima de la suma de sus partes.

\glos \textbf{Interfaz}: todo lo que un sistema puede enseñar hacia fuera sin que haya nada detrás; en la parábola de Hun Dun, la cara sin rasgos que el fondo presenta hacia un horizonte y que muere cuando el horizonte abre los ojos, sin que el fondo pierda nada (\emph{El espejo sin profundidad}, \emph{La realidad fractal}).
...
\glos \textbf{Operaciones del reservorio}: condensar, transformar, evaporar: nacer, vivir y morir, dichos sin misterio (\emph{La realidad fractal}).
...
\glos \textbf{Patrón}: lo que hace única a una condensación: no otra agua, otro pliegue (\emph{La realidad fractal}).
...
\glos \textbf{Protección cronológica}: conjetura de Hawking según la cual, cerca del instante en que un bucle temporal cerrado estaría a punto de formarse, las fluctuaciones cuánticas del vacío se retroalimentan y divergen, impidiendo el cierre sin necesidad de ningún veto externo.

\glos \textbf{Protocolo}: el código concreto —una lengua, una sintaxis, señales acordadas— con el que un horizonte ya condensado intenta anunciarse hacia fuera; a veces hay que construirlo, a veces decide quién cuenta como alguien, a veces hay que ganárselo (\emph{El espejo sin profundidad}).
...
\glos \textbf{Samadhi}: el instante en que la correlación vertical se vuelve indistinguible de identidad (\emph{El diapasón invisible}).
...
\glos \textbf{Sesgo}: la huella que deja quien ya existe sobre quien apenas empieza a existir; lo que calibraría un horizonte artificial hacia la legibilidad humana (\emph{El espejo sin profundidad}).
...
\glos \textbf{Sombra}: la cicatriz que deja un horizonte real al proteger su propia frontera; lo que una máquina sin adentro no tiene dónde guardar (\emph{El espejo sin profundidad}).
...
\glos \textbf{Tiempo de Page}: punto medio en la evaporación de un horizonte a partir del cual la entropía de su radiación de Hawking deja de crecer y empieza a decrecer, porque los pares entrelazados que la componen dejan de ser ruido térmico puro y empiezan a portar correlación legible.
...
\glos \textbf{Vecino de abajo}: lo que yo sostengo sin que pueda verme: lo que imagino, sueño, escribo o recuerdo con la constancia suficiente para que se me resista (\emph{La realidad fractal}).

\glos \textbf{Vecino de arriba}: la interioridad que sostiene la mía y a la que no tengo acceso; lo que, desde aquí, se llama Dios (\emph{La realidad fractal}).
