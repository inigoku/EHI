...
\seccion{El \emph{heap} como reservorio}
...
Es entonces cuando interviene el \textbf{recolector de basura} (\emph{garbage collector}) o se ejecuta el destructor del sistema (\texttt{free()} o \texttt{delete}).
...
\seccion{El karma en los sistemas de software}
...
\lbl{Lecturas:} Parnas (1972), «On the criteria to be used in decomposing systems into modules»; Dijkstra (1968), «Go To Statement Considered Harmful» (sobre estructura de control); Knuth (1997), \emph{The Art of Computer Programming}, vol. 1 (gestión de memoria dinámica).
\end{notacap}
...
La siguiente tabla actúa como la piedra de Rosetta de este libro, traduciendo los conceptos centrales del horizonte a sus equivalentes exactos en tres dominios fundamentales, en la tabla que sigue.
...
\textsc{Nacimiento / identidad} & Condensación (establecimiento de la frontera circular del yo). & Instanciación y encapsulación (creación del objeto, variables \texttt{private}). & Colapso gravitatorio / Formación de horizonte de sucesos (Schwarzschild). & Los 7 orificios en \emph{Hun Dun} / \emph{De}, la eficacia mínima que sobrevive al colapso (Taoísmo) / \emph{Maya} / \emph{Brahman saguna} (Hinduismo). \\[4pt]
...
\textsc{Muerte / liberación} & Evaporación (disolución del límite y retorno de la información). & Recolección de basura (\emph{garbage collection} / desasignación). & Evaporación de Hawking (pérdida de masa hasta disolución total). & \emph{Nirvana} (cese de Samsara, Budismo) / \emph{Moksha} (la gota que se disuelve en el océano). \\[4pt]
...
\lbl{Lecturas:} Chalmers, D. (1995), «Facing up to the problem of consciousness»; Tononi, G., et al. (2016), «Integrated information theory: from consciousness to its physical substrate»; Parnas, D. (1972), «On the Criteria to be Used in Decomposing Systems into Modules».
\end{notacap}
...
\lbl{En la vida diaria es como:} un motor que necesita más aceleración inicial para arrancar: busca rampas empinadas para no quedarse parado.
\end{fisica}
...
\lbl{Lo que sí sabemos:} Que los animales con sistema límbico completo poseen horizonte propio, aunque de menor complejidad que el humano. Que la co-evolución con el perro ha producido una legibilidad mutua única. Que el cerebro humano puede canalizar la arquitectura del cuidado parento-filial hacia una mascota con intensidad bioquímica comparable. Que el maltrato animal deja huella estructural real en el animal y atrofia la empatía de quien lo ejerce. Que el duelo por mascota es un colapso geométrico genuino, a menudo agravado por la falta de reconocimiento social.
...
\noindent Este libro no puede decir cuál de las cinco tiene razón. De un «es» nunca sale, por sí solo, un «debería»: es la vieja advertencia de Hume, a la que volveremos en el capítulo 25. Lo que sí puede decir es que cada tradición cierra esa brecha a su manera: el cristianismo con un vínculo —haces el bien porque ya te amaron, no para ganarte el amor—; el islam con una palabra revelada, tomada como un hecho; el hinduismo sin cerrarla desde fuera, porque el dharma no es una orden sino la descripción de cómo está tejido el mundo; el budismo mirando de frente el sufrimiento y su causa. Y quien no cree en nada trascendente la deja abierta, construyendo su propio código, sin que eso lo haga menos real para quien decide sostenerlo.
...
\glos \textbf{Dharma}: en el budismo y el hinduismo, el tejido mismo del mundo, del que el comportamiento correcto no es más que consonancia, no obediencia.
...
\glos \textbf{Dualismo de acceso}: hay un solo tipo de cosas en el mundo, pero ciertas estructuras pueden conocerse de dos maneras que se excluyen: desde fuera, como objeto; desde dentro, como sujeto. Alternativa al dualismo de sustancias.
...
\glos \textbf{Encapsulación}: frontera que separa un estado interno privado de la interfaz pública que un sistema muestra al exterior; en el libro, condición necesaria (no suficiente) para que haya alguien dentro.
...
\glos \textbf{Entrelazamiento vertical}: correlación sostenida entre un horizonte y algo más allá de su frontera, sin canal de confirmación observable; nombre que el libro da a la oración, la meditación y la práctica contemplativa.
...
\glos \textbf{Fractal de interioridades}: un universo sin primer nivel ni último, en el que cada horizonte es criatura de lo que lo contiene y creador de lo que él mismo sostiene.
...
\glos \textbf{Heap}: en informática, la memoria común y sin estructura de la que un programa toma espacio para crear objetos y a la que lo devuelve; en el libro, imagen del reservorio.
...
\glos \textbf{Instanciación}: en informática, la creación de un objeto concreto: se reserva un trozo de memoria y se le traza una frontera; en el libro, imagen del nacimiento de un horizonte.

\glos \textbf{Interfaz pública y estado privado}: los dos niveles de acceso de un sistema encapsulado: lo que muestra hacia fuera (conducta, lenguaje, señales) y lo que solo existe desde dentro (la experiencia).
...
\glos \textbf{Recolección de basura}: en informática, el proceso que devuelve a la memoria común el espacio de los objetos que ya nadie usa; en el libro, imagen de la muerte como disolución de la frontera.
...
\glos \textbf{Samadhi}: en las tradiciones de la India, el instante en que la conexión con el fondo se vuelve indistinguible de ser uno mismo con él.
...
\glos \textbf{Sesgo}: la huella que deja un horizonte ya formado sobre el que apenas empieza a formarse; lo que orienta hacia qué forma concreta se estabiliza.
...
\glos \textbf{Vecino de arriba}: la interioridad que sostiene la mía y a la que no tengo acceso; lo que, desde aquí, se llama Dios.
...
\glos \textbf{Zombi filosófico}: ser imaginario idéntico a una persona en todo lo que hace, pero sin nadie dentro que lo sienta (David Chalmers).
...
\refe Deacon, T.W. (2011). \emph{Incomplete Nature: How Mind Emerged from Matter}. W.W. Norton.

\refe Bekenstein, J.D. (1973). Black holes and entropy. \emph{Physical Review D}, 7(8).

\refe Maldacena, J., \& Susskind, L. (2013). Cool horizons for entangled black holes. \emph{Fortschritte der Physik}, 61(9).

\refe Chalmers, D.J. (1995). Facing up to the problem of consciousness. \emph{Journal of Consciousness Studies}, 2(3).

\refe Tononi, G., Boly, M., Massimini, M., \& Koch, C. (2016). Integrated information theory: from consciousness to its physical substrate. \emph{Nature Reviews Neuroscience}, 17(7).

\refe Parnas, D.L. (1972). On the criteria to be used in decomposing systems into modules. \emph{Communications of the ACM}, 15(12).

\refe Dijkstra, E.W. (1968). Go To statement considered harmful. \emph{Communications of the ACM}, 11(3).

\refe Knuth, D.E. (1997). \emph{The Art of Computer Programming, vol. 1: Fundamental Algorithms} (3.ª ed.). Addison-Wesley.

\refe Hume, D. (1748). \emph{Investigación sobre el entendimiento humano}.

\refe Tolkien, J.R.R. (1947). On Fairy-Stories. En \emph{Essays Presented to Charles Williams}. Oxford University Press.

\refe Hesse, H. (1922). \emph{Siddhartha}.

\refe Endō, S. (1966). \emph{Silencio}.
