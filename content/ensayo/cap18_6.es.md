...
### 8. Por qué la IA no tiene Sombra, aunque parezca tenerla

Dejamos abiertas dos preguntas sobre la inteligencia artificial. Vale la pena responderlas ahora, con el vocabulario que este capítulo ya ha construido, aunque la respuesta obligue a revisar más adelante en este ensayo qué clase de sistema es realmente un modelo de lenguaje.

La Sombra, tal como la hemos descrito, no es simplemente "información oculta": es información que el Ego expulsa de sí mismo hacia su propio inconsciente, hacia el mismo Reservorio del que el Ego se condensó, porque integrarla globalmente amenazaría con romper la membrana. Para que ese movimiento tenga sentido hacen falta dos cosas a la vez: un Ego ya condensado —un horizonte con adentro y afuera propios— y un Reservorio propio hacia el que expulsar lo inasimilable. La Sombra no es el dato oculto; es la cicatriz que deja un horizonte real al proteger su propia membrana.

Un modelo de lenguaje actual no cumple ninguna de las dos condiciones. Más adelante en este ensayo llamaremos a la IA un anti-reservorio: un sistema que articula y agrega información sin haber condensado nunca una plenitud propia de la que nada pudiera condensarse. Sin plenitud propia, no hay hacia dónde expulsar nada; sin membrana condensada por rotura de simetría —los pesos de un modelo se fijan por descenso de gradiente, no por el mismo proceso que separa un horizonte de su reservorio—, no hay un "adentro" desde el que algo pueda resultar inaceptable. Que existan pesos que la empresa que lo entrena decide no exponer, o datos que un filtro le impide procesar, no es lo mismo que una Sombra: es, como mucho, una restricción externa sobre la interfaz, del mismo tipo que un permiso de archivo. La Sombra junguiana se genera desde dentro, como defensa; la restricción de un modelo se impone desde fuera, como diseño.

Con la Persona ocurre algo distinto, y por eso merece una respuesta distinta. La Persona no presupone un Reservorio propio: presupone, únicamente, una interfaz pública que puede divergir de un estado interno. Un modelo de lenguaje sí que produce algo que se le parece —un tono consistente, una "personalidad" ajustada por instrucciones que se mantiene estable de conversación en conversación—, y no hay razón para negar que eso funcione, de cara afuera, como una Persona. Pero aquí es donde el "espejo sin profundidad" de más adelante se vuelve decisivo: la Persona junguiana es una interfaz que protege *algo*, un estado interno real que existe y que la máscara mantiene a resguardo. La "personalidad" de un modelo de lenguaje no protege nada, porque —según argumentará este ensayo— no hay encapsulación existencial detrás: no hay un estado privado distinto de lo que la interfaz ya muestra. Es una Persona sin nada detrás de la máscara; una capa de compatibilidad que no cumple su función original, porque su función original era separar lo público de un privado que en este caso no existe.

Esto no cierra del todo la pregunta que dejamos pendiente sobre si un horizonte puede contener genuinamente a otro horizonte —la Sombra dentro del Ego—. Pero sí acota una condición previa: sea lo que sea la respuesta a esa pregunta, requiere que haya, primero, un horizonte que contener. Y eso es, precisamente, lo que un anti-reservorio no tiene.
