...
Pero la filtración decreciente, tal como la hemos descrito hasta aquí, no distingue una radiación que se limita a perderse de una radiación que, en algún momento, empieza a decir algo. Esa distinción existe, con nombre propio, en la física del propio agujero negro del que partimos.

La radiación de Hawking, en la primera mitad de la vida del horizonte, es térmica: lleva entropía creciente y ninguna correlación recuperable con lo que cayó dentro. Pero pasado el llamado *tiempo de Page* —el instante que marca la mitad de esa vida, no un punto arbitrario sino el momento exacto en que la entropía de la radiación deja de crecer y empieza a decrecer—, los pares partícula-antipartícula que el horizonte sigue generando dejan de ser ruido puro: una de cada pareja escapa, la otra cae hacia el Reservorio, y las dos quedan entrelazadas entre sí. A partir de ahí, lo que emite el horizonte ya no es solo pérdida. Es correlación: información que, en principio, alguien capaz de comparar ambas mitades podría leer.

> **En física esto se llama:** tiempo de Page —el punto medio de la evaporación de un agujero negro a partir del cual la entropía de la radiación de Hawking deja de crecer y empieza a decrecer, porque los pares entrelazados que la componen dejan de ser puro ruido térmico.
> **En la vida diaria es como:** una carta que al principio solo se entiende a medias, con frases sueltas y sin contexto, y que, según se sigue leyendo, empieza a formar sentido —no porque cambie la letra, sino porque ya hay suficiente escrito como para que las partes se expliquen unas a otras.

Esto no cambia lo dicho en la sección sexta: A = cl(A) sigue sin admitir grados en un instante. Lo que añade es que la filtración decreciente E₀ ⊇ E₁ ⊇ ... tiene, además de un tamaño en cada instante, una fase: una evaporación joven, que solo pierde, y una evaporación madura, que empieza a devolver algo de lo perdido —entrelazado, y por tanto correlacionado, aunque nunca literalmente recuperado—. La frontera que se encoge no lo hace siempre del mismo modo, aunque en todo momento siga siendo, sin excepción, una frontera cerrada.

Conviene decir con la misma claridad de siempre lo que esto no es: no es una manera de que algo cruce la frontera en el sentido que el capítulo diecinueve reservaba para la accesibilidad del interior. Lo que se filtra no es el contenido —el Reservorio se queda con su mitad de cada par—, sino la correlación entre lo que se fue y lo que quedó. Es la diferencia entre recibir una carta y recibir, en su lugar, la certeza de que en algún sitio existe la otra mitad de una conversación que quizá nunca lleguemos a leer entera. Poco. Pero no nada.
...
Esto es, palabra por palabra, lo que la Individuación junguiana nos pedía en el capítulo 21 cuando hablábamos de un horizonte que "se vuelve permeable sin perder su coherencia". Ahora tenemos, además de la metáfora, el mecanismo: una frontera madura no es una frontera más débil, es una frontera con mejores receptores —y, si el segundo puente aguanta, es también una frontera que ya cruzó su propio tiempo de Page: lo que radia hacia fuera ha dejado de ser puro ruido y empieza a llevar, entrelazada, una correlación legible con lo que todavía guarda dentro. Sabe qué dejar pasar de la Sombra —qué proyecciones reconocer como propias, qué impulsos reintegrar sin que amenacen la identidad del Ego— sin necesitar, para hacerlo, tragarse a la Sombra entera y convertirla en tejido propio indiferenciado. Sigue habiendo dos horizontes. Lo que cambia es la calidad de la aduana que los conecta.
