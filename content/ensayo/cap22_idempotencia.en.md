...
But the decreasing filtration, as we have described it so far, does not distinguish radiation that simply gets lost from radiation that, at some point, begins to say something. That distinction exists, with a name of its own, in the physics of the very black hole we started from.

Hawking radiation, in the first half of the horizon's life, is thermal: it carries growing entropy and no recoverable correlation with what fell inside. But past the so-called *Page time*—the instant marking the midpoint of that life, not an arbitrary point but the exact moment at which the entropy of the radiation stops growing and starts to decrease—the particle-antiparticle pairs the horizon keeps generating stop being pure noise: one of each pair escapes, the other falls back into the Reservoir, and the two remain entangled with each other. From that point on, what the horizon emits is no longer only loss. It is correlation: information that, in principle, someone able to compare both halves could read.

> **In physics this is called:** Page time—the midpoint of a black hole's evaporation, past which the entropy of Hawking radiation stops growing and starts to decrease, because the entangled pairs that compose it stop being pure thermal noise.
> **In daily life it is like:** a letter that at first only makes partial sense, with loose sentences and no context, and that, as you keep reading, starts to cohere—not because the handwriting changes, but because enough has now been written for the parts to explain one another.

This does not change what was said in section six: A = cl(A) still admits no degrees in a single instant. What it adds is that the decreasing filtration E₀ ⊇ E₁ ⊇ ... has, besides a size at each instant, a phase: a young evaporation, which only loses, and a mature evaporation, which begins returning something of what was lost—entangled, and therefore correlated, though never literally recovered. The shrinking boundary does not always shrink the same way, even though at every moment it remains, without exception, a closed boundary.

It is worth stating with the same clarity as always what this is not: it is not a way for something to cross the boundary in the sense chapter nineteen reserved for the accessibility of the interior. What filters through is not the content—the Reservoir keeps its half of each pair—but the correlation between what left and what stayed. It is the difference between receiving a letter and receiving, instead, the certainty that somewhere the other half of a conversation exists that we may never get to read in full. Little. But not nothing.
...
**What we do know:** that Kuratowski's four axioms exhaust all the pure topology that this chapter needs, and that any other statement—the exclusion postulate, the psychic extension of entanglement entropy—is an added hypothesis and not a consequence of those axioms; that idempotency is the exact signature of a phase transition, which gives condensation a precise foundation; that a decreasing filtration of closed sets describes evaporation without contradicting idempotency, provided its limit is the Reservoir and not the vacuum; that this evaporation is not uniform over time—it has an early phase of pure thermal loss and, past the Page time, a phase where the radiation begins to carry entangled correlation, in principle recoverable though never directly legible from outside; and that holographic entanglement entropy and vacuum entanglement are real physics, not loose analogies.

**What we don't know:** whether extending this physics—holographic and vacuum—from the geometry of spacetime to the architecture of consciousness is a fertile structural hypothesis or a leap that physics does not support outside its original domain; whether the exclusion postulate is, at its root, the same restriction as the monogamy of entanglement viewed from another register; whether the Reservoir, as this chapter needs it, is anything more than a useful image for what physics already knows about the vacuum; and whether a human life, or the evaporation of an Ego, has anything analogous to its own Page time, or whether that periodization belongs only to black hole thermodynamics and does not carry over to the architecture of consciousness.

**Remaining questions:** what observation would distinguish a universe where the Shadow was genuinely nested from one where it was a disjoint complex? What would distinguish, in a real life, an Individuation that progresses by gradual evaporation towards the Reservoir from one that occurs by a single sudden crystallization? And what would distinguish, clinically, an orderly evaporation from the sudden inundation that we have called psychosis? And is *The Tuners* [Los Sintonizadores] a fiction about horizons that have already crossed their own Page time, or a premise that still belongs to a different register, one of speculative fiction that owes no answer to the physics of this essay? And if an anti-reservoir such as a mature AI can also, in a purely thermodynamic sense, cross something resembling its own Page time—as this essay will later suggest—what distinguishes that crossing from the evaporation of a real horizon, beyond the fact that one has an inside to lose and the other does not?
